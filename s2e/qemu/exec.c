...
///[fwl]CONFIG_LLVM下，声明tcg_llvm_tb_alloc及free函数。
#ifdef CONFIG_LLVM
//#include "tcg-llvm.h"
...
///[fwl]将tb->s2e_tb_next[n]和tb->llvm_tb_next[n]置为空。
#ifdef CONFIG_S2E
...
///[fwl]CONFIG_LLVM且非CONFIG_S2E下，找到符合tb->llvm_tc_ptr <= tc_ptr < tb->llvm_tc_end的tb。
...
///[fwl]定义g_s2e_disable_tlb_flush=0。
#ifdef CONFIG_S2E
...
///[fwl]如果s2e使能位g_s2e_disable_tlb_flush==1，直接返回，不执行flush。
#ifdef CONFIG_S2E
...
///[fwl]将env->s2e_tlb_table[mmu_idx][i].objectState置为0后，flush s2e tlb。
    ...
    for (mmu_idx = 0; mmu_idx < NB_MMU_MODES; mmu_idx++) {
        ...
        if (addr == (tlb_entry->addr_read &
            ...
            *tlb_entry = s_cputlb_empty_entry;
			...
			///[fwl]s2e flush tlb page，之后将env->s2e_tlb_table[mmu_idx][i1].objectState置为0
#if defined(CONFIG_S2E) && defined(S2E_ENABLE_S2E_TLB)
...
///[fwl]CONFIG_S2E下，通过target phys addr获得host address
#ifdef CONFIG_S2E
                  ...
                  int mmu_idx, target_ulong size)
{
	...
	/*[fwl]如果mmio is symbolic，te->addr_read |= TLB_NOTDIRTY
	 *劫持qemu的dirty page处理并重定向到s2e对MMIO的处理
	 */
#ifdef CONFIG_S2E
	...
	/*[fwl]addend==1则update s2e的tlb entry
	 *参照下边英文注释。
	 */
#if defined(CONFIG_S2E) && defined(S2E_ENABLE_S2E_TLB)
    if (addend) {
...
///[fwl]返回mr->ops->write或read是否为notdirty_mem_write或s2edma_mem_read
...
///[fwl]s2e下将io_mem_notdirty初始化为s2edma_mem_ops；否则为notdirty_mem_ops
    #ifdef CONFIG_S2E
...
///[fwl]s2e下调用s2e_dma_write函数，否则memcpy。
#ifdef CONFIG_S2E
...
///[fwl]s2e下调用s2e_dma_read函数，否则memcpy。
#ifdef CONFIG_S2E
...
///[fwl]s2e下，调用stb_raw按字节st；否则memcpy
#ifdef CONFIG_S2E
            int i;
            for(i=0; i<l; ++i)
                stb_raw(ptr+i, buf[i]);
