...
/*[chy]
 *  DisasContext: 反编译 guest binary 所用的数据结构
 * s2e对DisasContext数据结构增加的成员变量
 */
#ifdef CONFIG_S2E
    void *cpuState;
...
/*[chy]
 * 定义了SET_TB_TYPE宏
 * 实现s2e_translate_compute_reg_mask_end函数，含义???
 */
#ifdef CONFIG_S2E
...
#endif
/*[chy]
 *  调用s2e_on_translate_instruction_end函数，通知s2e翻译指令结束
 */
#ifdef CONFIG_S2E
...
/*[chy]
 *  调整了执行顺序，中间插入S2E新增加的函数gen_intr_end，通知s2e翻译了一条指令
 */
#ifdef CONFIG_S2E
        //s->enable_jmp_im = 0;
        gen_jmp_im(s, eip);
...
static void gen_eob(DisasContext *s)
{
//[chy] 执行S2E新增加的函数gen_intr_end，通知s2e翻译了一条指令
#ifdef CONFIG_S2E
    gen_instr_end(s);
#endif
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
    ...
    int rex_w, rex_r;
/*[chy]
 * disas_insn 反汇编 guest binary 成 TCG IR
 * 生成一条表示翻译指令开始的QEMU specific tcg指令debug_insn_start
 */
...
/*[chy]
 * tmp0=pc_start - s->cs_base ; cpu_env->eip = tmp0; temp1=cpu_env->s2e_icount;  temp1++; cpu_env->s2_icount=temp1
 * 生成设置cc_op的指令
 */
#ifdef CONFIG_S2E
...
//[chy]-------------------------------
 ...
 reswitch:
    switch(b) {
/*[chy]
 * 0x13f 是s2e_op的custom opcodes
 * 调用CorePlugin.cpp:s2e_tcg_emit_custom_instruction生成s2e的custom opcodes
 */
    case 0x13f: /* s2e_op */
        {
...
//[chy]  s->tb->s2e_tb_type = TB_CALL_IND
...
//[chy]  s->tb->s2e_tb_type = TB_CALL_IND
        ...
        do_lcall:
            ...
            break;
        case 4: /* jmp Ev */
//[chy]  s->tb->s2e_tb_type = TB_JMP_IND
...
//[chy]  s->tb->s2e_tb_type = TB_JMP_IND
    ...
    case 0xc2: /* ret im */
/*[chy]
 *  s->tb->s2e_tb_type = TB_RET, 调用s2e_on_translate_jump_start，通知s2e jmp开始
 */
        SET_TB_TYPE(TB_RET);
...
//[chy]  s->tb->s2e_tb_type = TB_COND_JMP
    ...
    case 0x1ae:
        ...
        case 3: /* stmxcsr */
            ...
            if (op == 2) {
                gen_op_ld_T0_A0(OT_LONG + s->mem_index);
/*[chy]
 *   qemu: tcg_gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, mxcsr));
 *   s2e: below
 */
                tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
...
/*[chy]
 * s2e增加必要的初始化信息  tmp1=tb; cpu_env->s2e_current_tb=tmp1
 */
#ifdef CONFIG_S2E
    dc->enable_jmp_im = 1;
    dc->cpuState = env;
...
//[chy]--------------------------------------------------
    ...
    for(;;) {
        ...
        if (num_insns + 1 == max_insns && (tb->cflags & CF_LAST_IO))
            gen_io_start();
/*[chy]
 * s2e进行必要的初始化，调用s2e_on_translate_instruction_start通知s2e 指令翻译开始
 */
#ifdef CONFIG_S2E
        dc->insPc = pc_ptr;
        dc->done_instr_end = 0;
