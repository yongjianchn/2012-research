...
/*[chy] change CPUState to CPUArchState 
 * CPUArchState 表明CPU的状态，这里特指i386 CPU
 */
...
//[chy] 全局替换 CPUState to CPUX86State
...
/*[chy] 
 *  ??? CFLAGS_MASK MFLAGS_MASK 的翻译
 *  mflags 是CPUX86State中的成员变量，是Mode and control flags from eflags 
 */
...
//[chy]--------------------------------------
    ...
    uint64_t xcr0;
/*[chy] 
 *  增加了成员变量 tpr_access_type ???
 */
    TPRAccess tpr_access_type;
} CPUX86State;

/*[chy] 
 *  提供S2E版本的具体读写寄存器和eflags的实现
 *  提供 RR/WR_cpu函数，调用的是s2e_read/write_register_concrete
 *  修改cpu_get/set_eflags实现，注意用的是CPUX86State中的mflags成员变量
 */
...
//[chy]  change CPUState to CPUX86State
...
{
/*[chy] 
 *  增加调用让s2e感知到CPL变化的函数s2e_on_privilege_change
 */
#ifdef CONFIG_S2E
...
/*[chy] 
 *  通过调用RR/WR_cpu来修改对各个寄存器和符号位的读写操作
 */
#undef EAX
...
//[chy］添加新函数定义 cpu_report_tpr_access ???
