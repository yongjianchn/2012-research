...
/*[chy]
 * 定义了重要的CPU状态全局变量env, struct CPUX86State定义在cpu.h中
 */
...
#else
/*[chy]
 * 重新定义parity_table rclw_table rclb_table 位于qemu/s2e/S2EExecutor.cpp
...
/*[chy]
 * s2e不需要 global_cpu_lock,和函数helper_lock/unlick
 */
#ifndef S2E_LLVM_LIB
...
#include "softmmu_template.h"
/*[chy]
 *  扩展的是_mmu_symb _raw_symb 等处理符号值的函数
 */
#if defined(CONFIG_S2E) && !defined(S2E_LLVM_LIB)
#undef MMUSUFFIX
              ...
              void *retaddr)
{
    ...
    unsigned long pc;
/*[chy]
 * 调用s2e_on_tlb_miss，记录tlb miss 情况
 */
#ifdef CONFIG_S2E
...
//[chy]-----------------------------------
    ...
    if (ret) {
/*[chy]
 * s2e传递的是page addr（总是具体值）而不是实际的地址（因为此地址可能被符号化）给cpu_x86_handle_mmu_fault
 */
#ifdef CONFIG_S2E
...
//[chy] s2e_ensure_symbolic这个空函数有啥用???
#if defined(CONFIG_S2E) && defined(S2E_LLVM_LIB)
