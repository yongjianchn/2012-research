 ...
 *
 */
///[fwl]将所有CPUState->CPUArchState,longjmp->s2e_longjmp,setjmp->s2e_setjmp。
...
/*[fwl]
 *include s2e_qemu.h,tcg_llvm.h头文件，定义const变量has_llvm_engine=1。
 *定义generate_llvm=0,execute_llvm=0。
 */
#ifdef CONFIG_S2E
                             ...
                             TranslationBlock *orig_tb)
{
	...
	/*[fwl]
	 *CONFIG_LLVM且非CONFIG_S2E下，断言execute_llvm==0。
	 */
	...
	/*[fwl]
	 *给env中s2e相关成员赋值，使用s2e qemu对tb执行函数；否则，使用qemu对tb执行函数。
	 */
#ifdef CONFIG_S2E
...
int cpu_exec(CPUArchState *env)
{
	...
	///[fwl]TARGET_I386下，都被注释掉，因为eflags总是这样的格式。
...
#error unsupported target CPU
    ...
    for(;;) {
        if (s2e_setjmp(env->jmp_env) == 0) {
			...
			/*[fwl]
			 *断言g_s2e_state,env异常不为EXCP_S2E
			 *调用函数完成s2e qemu对tb块的执行。
			 */
            #ifdef CONFIG_S2E
            assert(g_s2e_state);
    ...
    defined(TARGET_MICROBLAZE) || defined(TARGET_LM32) || defined(TARGET_UNICORE32)
                        ...
                        if ((interrupt_request & CPU_INTERRUPT_SMI) &&
                            ...
                            next_tb = 0;
                                   ...
                                   (((env->hflags2 & HF2_VINTR_MASK) && 
                                    ...
                                    (!(env->hflags2 & HF2_VINTR_MASK) && 
                                     (env->mflags & IF_MASK &&		///[fwl]eflags->mflags
...
#if defined(TARGET_I386)		///[fwl]将env->eflags的赋值注释掉。
				...
				/*[fwl]
				 *CONFIG_S2E下，使用s2e qemu对tb的执行函数，
				 *CONFIG_LLVM下，如果execute_llvm==1,使用tcg_llvm_qemu_tb_exec函数，
				 *否则，使用tcg_qemu_tb_exec函数。
				 */
                /* execute the generated code */
